		...
		<div class="subody">
			...
			<div class="info">
				...
				<div id="hiddenContent">
					mommefatale은 항상 고객의 소리를 들을 준비가 되어있습니다. <br>
					언제든지 문의주시면 24시간 친절히 답변 드리겠습니다.
			...
			<div class="info">
				...
				<div id="hiddenContent3">
					배송전 주문변경 및 배송전 주문취소는 오후 3시 이전에 고객센터(1234-5678)나 Q&amp;A로 연락주시면 처리 가능합니다. <br>
					오후 3시 이후에 요청하신 주문변경은 제품포장 및 송장출력이 완료된 상태라서 주문변경/주문취소가 불가능합니다.
				</div>
			</div>
			<div class="info">
				...
				<tbody>
					...
					<c:if test="${count != 0 }">
						<c:forEach var="vo" items="${vo}" varStatus="i">
							<tr>
								<td><c:set var="num" value="${number-i.index}" scope="page" />${num}</td>
								<!-- 페이지에서만 유효한 글 번호 -->
								<!-- <td>${vo.no}</td>-->
								<!-- 실제 입력되는 순서의 글번호 -->
