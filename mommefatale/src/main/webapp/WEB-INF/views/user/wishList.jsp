...
<script type="text/javascript">
	function checkboxArr(check,all){
		var form = document.wishListForm;
		var checkArr=[];
		if(all){
			$("input[name='wishCheck']").each(function(){
			...
			$("input[name='wishCheck']:checked").each(function(){
				checkArr.push($(this).val());
			});
			$("input[name='arr']").val(checkArr);
		...
		if(check){
			...
			$(form).attr("action", "wishdelete.do");
		}
		form.submit();
	}
	
	
	function buynow(check,wish_no){
		var form = document.wishListForm;
		var checkArr=[];
		checkArr.push(wish_no);
		$("input[name='arr']").val(checkArr);
		if(check){
			$(form).attr("action", "payment.do");
		}else{
    ...
    <input type="hidden" name="arr" value="">
      <table class="menu">
       ...
       <c:forEach var="vo" items="${wishlist }">
        <tr class="itemlist">
            <td class="firstline"><input type="checkbox" name="wishCheck" class="checking" value="${vo.wish_no}"/> </td>
            ...
            <td>${vo.saving}p</td>
            <td>${vo.fee}원</td>
            <td>${vo.item_discount*vo.wish_count }원</td>
            <td width="80" class="firstline2">
                <div class="but" onclick="buynow(true,${vo.wish_no})"><a href="#" class="te">바로구매 ▶</a></div>
            </td>
        </tr>
       </c:forEach>
      </table>
